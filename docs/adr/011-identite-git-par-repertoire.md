...
```ini
[includeIf "gitdir:~/Projects/Septeo/"]
    path = ~/Projects/Septeo/.gitconfig
```

L'identité est donc fonction du **répertoire du dépôt**, résolue par git à chaque
commande, et non du profil résolu à l'apply.

`~/Projects/Septeo/.gitconfig` n'est pas versionné dans ce dépôt : il porte les
...
- La convention devient une contrainte de rangement : un dépôt professionnel
  **doit** vivre sous `~/Projects/Septeo/`. Ailleurs, il hérite silencieusement de
...
- **Versionner `~/Projects/Septeo/.gitconfig`** ici : y ferait entrer les
