...
- **Septeo** (profil `pro`) — `modelo-suite`, `modelo-hub`, `modelo-flow` :
  monorepos TypeScript. Session isolée via `claude-septeo`
  (`CLAUDE_CONFIG_DIR=~/.claude_septeo`).
...
- **Bellman** — React, Node, PostgreSQL, Prisma, TypeScript strict.
  Environnements via `doppler` (`doppler-api-prod`, `-dev`, `-local`, `-feat-0x`).
- **Axion** — `Amure`, `rustless-2`.
- **Perso** — `ulm-club-60-calculator`, `vcards-saas`, ces dotfiles.
