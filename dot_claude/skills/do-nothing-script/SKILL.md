...
```text
/do-nothing-script turn the new-machine bootstrap into a script
/do-nothing-script automate step 3 of scripts/provision-user
```
...
7. Automate one step at a time: replace its body with real code, keep its name and position, keep the
   rest manual. Never block shipping the script on automating everything.
...
step_fetch_pat() {
  echo "Open Bitwarden, entry github.com, custom field 'Dotfiles token'."
  echo "Then run: export GH_PAT=<value>"
  wait_for_enter
}

step_apply_chezmoi() { chezmoi apply; }  # automated
